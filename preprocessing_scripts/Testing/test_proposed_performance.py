...
for ind, data in test_result_df.iterrows():
    try:
        ...
        input_features = torch.cat((video_feature_npy, user_history_npy, text_feature_npy), dim=0)
...
if os.path.exists('proposed_result.csv'):
    os.remove('proposed_result.csv')
test_result_df.to_csv('proposed_result.csv')
...
if not os.path.exists('proposed_result.csv'):
    raise FileNotFoundError('result not available')


df = pd.read_csv('proposed_result.csv')
...
"""
Proposed
mAP : 0.07832976013622392
nMSD: 0.5303048553294207
R@5 : 0.40625
"""