...
# Define the model
model = Net().cuda() if params_cuda else Net()
...
for ind, data in test_result_df.iterrows():
    try:
        ...
        distance = torch.nn.functional.cosine_similarity(video_feature_npy, user_history_npy, dim=1, eps=1e-8)

        input_features = torch.cat((video_feature_npy, distance.reshape(4096,1), text_feature_npy), dim=0)
...
if os.path.exists('proposed_with_distance_result.csv'):
    os.remove('proposed_with_distance_result.csv')
test_result_df.to_csv('proposed_with_distance_result.csv')
...
if not os.path.exists('proposed_with_distance_result.csv'):
    raise FileNotFoundError('result not available')


df = pd.read_csv('proposed_with_distance_result.csv')
...
"""
Proposed with distance
mAP : 0.0772244720389109
nMSD: 0.5260782094147952
R@5 : 0.40625
