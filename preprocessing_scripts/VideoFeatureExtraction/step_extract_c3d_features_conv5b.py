...
class C3D(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.relu(self.conv5b(h))  # shape (1, 512, 2, 7, 7)
        h = self.pool5(h)  # shape (1, 512, 1, 4, 4)
        h = h.view(-1, 8192)
...
class ExtractC3D:
    ...
    def __init__(self):
        ...
        self.FEATURE_DIR = os.path.join(self.DATA_DIR, 'feature_c3d_conv5b', 'video_features')
    ...
    def extract(self, data):
        feature_conv5b = torch.zeros((1, 512, 2, 7, 7))

        def copy_data(m, i, o):
            feature_conv5b.copy_(o.data)

        h = self.net.conv5b.register_forward_hook(copy_data)
        ...
        data = feature_conv5b.data.cpu().numpy()
        return data.flatten()
