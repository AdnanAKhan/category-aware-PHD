...
class HighlightDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(os.path.join(self.base_data_dir, self.textual_features[idx]), 'rb') as text_feature_file:
            text_feature = pickle.load(text_feature_file)
        user_history = np.loadtxt(os.path.join(self.base_data_dir, self.user_history_features[idx]), dtype=float)

        if self.transform:
            highlight_npy = self.transform(highlight_npy.reshape(4096, 1))
            non_highlight_npy = self.transform(non_highlight_npy.reshape(4096, 1))
            text_feature = self.transform(text_feature.reshape(100, 1))
            user_history = self.transform(user_history.reshape(4096, 1))

        return highlight_npy, non_highlight_npy, text_feature, user_history
...
def fetch_data_loader(types, data_dir, params):
    ...
    for split in ['train', 'val', 'test']:
        if split in types:
            if split == 'train':
                dl = DataLoader(HighlightDataset(data_dir, train_transformer, mode='train'),
                ...
                dl = DataLoader(HighlightDataset(data_dir, eval_transformer, mode='val'),
                                batch_size=params.batch_size,
                                shuffle=False,
                                num_workers=params.num_workers,
                                pin_memory=params.cuda)
            elif split == 'test':
                dl = DataLoader(HighlightDataset(data_dir, test_transformer, mode='test'),
                                batch_size=1,
                                shuffle=False,
                                num_workers=params.num_workers,
                                pin_memory=params.cuda)
